...
const server = http.createServer(async (req, res) => {
  ...
  if (action === 'verifysourcecode') { if (params.get('constructorArguements') !== '1234' || !params.get('sourceCode')) return send({ status: '0', result: 'bad verification form' }); const address = params.get('contractaddress'); if (address?.endsWith('dead')) return send({ status: '0', result: `Unable to locate ContractCode at ${address}` }); const count = (submits.get(address) ?? 0) + 1; submits.set(address, count); return send(count === 1 ? { status: '1', result: `guid-${address}` } : count === 2 ? { status: '0', result: 'Contract source code is already verifying' } : { status: '0', result: 'Contract source code already verified' }); }
