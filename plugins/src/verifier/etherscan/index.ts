...
function failed(code: string, retryable = true): PluginResponse<any> { return ok({ status: 'failed', retryable, detail: code }); }
function mapSubmit(json: any): PluginResponse<VerifyStatusResult> { const value = text(json); if (/already verified|already verifying/i.test(value)) return ok({ status: 'already-verified' }); if (json?.status === '1' && value) return ok({ status: 'pending', pollTicket: value }); return failed(/rate limit|nonce|temporar|server/i.test(value) ? 'RETRYABLE' : 'SUBMIT_FAILED', /rate limit|nonce|temporar|server/i.test(value)); }
function mapPoll(json: any): PluginResponse<VerifyStatusResult> { const value = text(json); if (/Pending in queue/i.test(value)) return ok({ status: 'pending' }); if (/Pass - Verified/i.test(value)) return ok({ status: 'verified' }); if (/Already Verified/i.test(value)) return ok({ status: 'already-verified' }); if (/Max rate limit|5\d\d|temporar/i.test(value)) return failed('RETRYABLE', true); return ok({ status: 'failed', retryable: false, detail: 'VERIFICATION_FAILED' }); }
