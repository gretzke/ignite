...
export default function DeployStepCard({ step, data, onMove }: { step: DraftDeployStep; data?: ArtifactData; onMove: (delta: number) => void }) {
  ...
  return <article className="card-milky p-4 grid gap-4">
    <header className="flex gap-2 items-start"><div className="flex-1"><h3 className="font-semibold">{contract?.contractName ?? decodeUrlEncodingForDisplay(step.contractId)}</h3><p className="mono-data text-muted">{contract?.sourcePath ? decodeUrlEncodingForDisplay(contract.sourcePath) : undefined}</p></div><button type="button" className="btn btn-sm btn-secondary" aria-label="Move step up" onClick={() => onMove(-1)}>↑</button><button type="button" className="btn btn-sm btn-secondary" aria-label="Move step down" onClick={() => onMove(1)}>↓</button></header>
