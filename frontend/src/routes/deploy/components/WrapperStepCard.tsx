...
import { setAcknowledgeUninitialized, setAcknowledgeUnverifiedBytecode, setArg, setChainArgOverride, setValue, setWrapperInitializer } from '../../../store/features/deployments/deployDraftSlice';
...
export default function WrapperStepCard({ step, implementationAbi, onMove }: { step: DraftDeployStep; implementationAbi?: unknown[]; onMove: (delta: number) => void }) {
  ...
  useEffect(() => {
    ...
    if (initialize.length === 1) dispatch(setWrapperInitializer({ stepId: step.id, key: initializerArg, selection: initialize[0].signature, value: { $encode: { contractId: implementationContract?.id ?? '', fn: initialize[0].signature, args: {} } } }));
  ...
  const setInitializer = (signature: string) => {
    ...
    dispatch(setWrapperInitializer({ stepId: step.id, key: initializerArg, selection: signature, value: signature ? { $encode: { contractId: implementationContract?.id ?? '', fn: signature, args: {} } } : '0x' }));
  };
  ...
  return <article className="card-milky p-4 grid gap-4">
    ...
    {type?.params.map((param) => { const input: AbiInput = { name: param.label, type: param.type === 'number' ? 'uint256' : param.type === 'boolean' ? 'bool' : param.type === 'select' ? 'string' : param.type }; const setParam = (value: unknown, chainId?: number) => chainId === undefined ? dispatch(setArg({ stepId: step.id, key: param.key, value })) : dispatch(setChainArgOverride({ stepId: step.id, chainId, key: param.key, value })); const field = (value: unknown, chainId?: number) => param.type === 'select' ? <label className="grid gap-1"><span className="text-sm font-medium">{param.label}</span><Select value={typeof value === 'string' ? value : undefined} options={param.options ?? []} onValueChange={(next) => setParam(next, chainId)} /></label> : <AbiArgField input={input} fieldKey={param.key} value={value} eligibleSteps={param.type === 'address' ? pointerSteps : undefined} autoDefault={param.type === 'boolean' && chainId === undefined} onChange={(value) => setParam(value, chainId)} />; return <div key={param.key} className="grid gap-2">{field(step.args?.[param.key])}{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain override</summary>{draft.chains.map((chainId) => <div key={chainId} className="mt-2">{field(step.argsPerChain?.[String(chainId)]?.[param.key], chainId)}</div>)}</details>}</div>; })}
    {initializerArg && <section className="grid gap-3"><label className="grid gap-1"><span className="eyebrow">Initializer</span><Select value={selected} options={[{ value: '', label: 'No initialization (empty calldata)' }, ...functions.map((item) => ({ value: item.signature, label: item.signature }))]} onValueChange={setInitializer} /></label>{selectedFunction?.item.inputs?.map((input, index) => { const key = input.name || `arg${index}`; const global = isEncode(step.args?.[initializerArg]) ? step.args![initializerArg] as Encode : undefined; return <div key={key} className="grid gap-2"><AbiArgField input={input} fieldKey={key} value={global?.$encode.args?.[key]} eligibleSteps={pointerSteps} onChange={(value) => setInitializerArg(key, value)} />{draft.chains.length > 1 && <details className="text-xs"><summary className="text-muted cursor-pointer">Per-chain override</summary>{draft.chains.map((chainId) => { const override = step.argsPerChain?.[String(chainId)]?.[initializerArg]; return <div key={chainId} className="mt-2"><AbiArgField input={input} fieldKey={key} value={isEncode(override) ? override.$encode.args?.[key] : undefined} eligibleSteps={pointerSteps} onChange={(value) => setInitializerArg(key, value, chainId)} /></div>; })}</details>}</div>; })}</section>}
