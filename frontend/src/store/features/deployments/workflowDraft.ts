import type { WorkflowDocument, WorkflowStep } from '@ignite/api';
import type { DeployDraftState } from './types';
import { cloneJson } from '../../../utils/cloneJson';

export function workflowDocumentFromDraft(draft: DeployDraftState): WorkflowDocument {
  if (!draft.workflowDocument) throw new Error('The draft is not bound to a workflow');
  ...
  const steps: WorkflowStep[] = draft.steps.map((step) => {
    ...
    if (step.kind === 'call') return cloneJson(signerFree) as unknown as WorkflowStep;
    ...
    if (extras?.strategy.kind === 'create') {
      if (originalStep?.kind === 'deploy' && originalStep.strategy) strategy = { kind: 'create' };
    } else if (extras?.strategy.kind === 'create2') {
      strategy = {
        ...cloneJson(extras.strategy),
        ...(extras.acknowledged ? { acknowledgeDeployed: cloneJson(extras.acknowledged) } : {}),
      };
    } else if (extras?.strategy.kind === 'plugin') {
      strategy = {
        ...cloneJson(extras.strategy),
        ...(extras.prepared ? {
          prepared: Object.fromEntries(Object.entries(extras.prepared).map(([chainId, prepared]) => [chainId, {
    ...
    return {
      ...
      ...(extras?.librariesPerChain ? { librariesPerChain: cloneJson(extras.librariesPerChain) } : {}),
  ...
  return {
    ...
    requiredPlugins: cloneJson(draft.workflowRequiredPlugins ?? original.requiredPlugins),
...
export function workflowDraftIsDirty(draft: DeployDraftState): boolean {
  ...
  try {
    return JSON.stringify(workflowDocumentFromDraft(draft)) !== JSON.stringify(draft.workflowDocument);
  } catch {
    return true;
  }
}
