...
import { appendSource, cascadeRemoveSource, changeSourceVersion, makeWorkflowDocumentSchema, mintSourceId, validateWorkflowClosure, type WorkflowDocument } from '@ignite/api';
...
function fixture(): WorkflowDocument {
  return makeWorkflowDocumentSchema().parse({
    ...
    steps: [
      ...
      { id: 'keep-deploy', kind: 'deploy', contractId: 'keep', libraries: { Rem: { kind: 'step', stepId: 'remove-deploy' } }, librariesPerChain: { '1': { Rem: { kind: 'step', stepId: 'remove-deploy' } } }, args: { ref: { $ref: { kind: 'step', stepId: 'remove-deploy' } }, encoded: { $encode: { contractId: 'remove', fn: 'set(address)', args: { target: address } } } }, argsPerChain: { '1': { encoded: { $encode: { contractId: 'remove', fn: 'set(address)', args: {} } } } }, strategy: { kind: 'plugin', pluginId: 'strategy', params: { encoded: { $encode: { contractId: 'remove', fn: 'set(address)', args: {} } } } } },
...
describe('workflow edit utilities', () => {
  it('cascades required references to a fixed point and clears every optional reference form', () => {
    ...
    expect(keep).toMatchObject({ id: 'keep-deploy', kind: 'deploy', libraries: {}, librariesPerChain: { '1': {} }, args: { encoded: { $encode: { fn: 'set(address)', args: { target: address } } } }, argsPerChain: { '1': { encoded: { $encode: { fn: 'set(address)', args: {} } } } } });
    expect((keep as Extract<WorkflowDocument['steps'][number], { kind: 'deploy' }>).strategy).toMatchObject({ kind: 'plugin', params: { encoded: { $encode: { fn: 'set(address)', args: {} } } } });
    expect(result.clearedRefs).toEqual(expect.arrayContaining([
      { stepId: 'keep-deploy', path: '$.steps[0].libraries.Rem.stepId' },
      { stepId: 'keep-deploy', path: '$.steps[0].librariesPerChain["1"].Rem.stepId' },
