...
const document = {
  ...
  sources: [
    {
      id: 'box',
      repo: { url: 'https://example.test/box.git', commit: 'a'.repeat(40) },
      frameworkId: 'foundry',
...
function fakeJobs() {
  const started: Array<{
    ...
    record: {
      id: string;
      state: 'queued' | 'failed' | 'succeeded' | 'cancelled';
    };
  }> = [];
  return {
    started,
    ...
    get: vi.fn(
      (id: string) => started.find((entry) => entry.id === id)?.record
    ),
    list: vi.fn(() => []),
  };
}
...
function makeService(overrides: Partial<WorkflowInstallServiceDeps> = {}) {
  ...
  const store = {
    ...
    writeInstalled: vi.fn(async (_profile, _key, _installed, guard) =>
      guard ? guard() : true
    ),
  ...
  const deps: WorkflowInstallServiceDeps = {
    ...
    registry: {
      list: vi.fn(async () => ({
        session: null,
        local: [{ pathOrUrl: '/workspace' }],
        cloned: [],
      })),
    } as never,
    store: store as never,
    repos: {
      removeVersionCheckout: vi.fn(async () => true),
      resolveWorkspacePath: vi.fn(async () => '/workspace'),
    } as never,
    pluginStatus: vi.fn(async (id, version) => ({
      id,
...
describe('WorkflowInstallService', () => {
  ...
  it('fences a changed document before creating a job', async () => {
    const { service, jobs } = makeService({
      readDocument: async () => ({ document, docHash: 'c'.repeat(64) }),
    });
    await expect(service.start('profile', request)).rejects.toMatchObject({
      statusCode: 409,
      code: 'WORKFLOW_DOC_CHANGED',
    });
  ...
  it('single-flights an install and retains its input pins synchronously', async () => {
    const { service, jobs } = makeService();
    await expect(service.start('profile', request)).resolves.toEqual({
      jobId: 'job-1',
      attached: false,
    });
    await expect(service.start('profile', request)).resolves.toEqual({
    ...
    expect(service.activeAttemptPins('profile')).toEqual([
      { url: 'https://example.test/box.git', commit: 'a'.repeat(40) },
    ]);
  });
  ...
  it('clears queued-cancel pins through onSettled without running the job', async () => {
    ...
    await jobs.started[0].opts?.onSettled?.({
      id: 'job-1',
      state: 'cancelled',
    } as never);
    expect(service.activeAttemptPins('profile')).toEqual([]);
  ...
  it('persists a failed attempt before rejecting with the result in error details', async () => {
    ...
    await expect(jobs.started[0].runner(ctx)).rejects.toMatchObject({
      ...
      details: {
        sources: [{ id: 'box', status: 'failed', code: 'ARTIFACT_NOT_FOUND' }],
      },
    });
    expect(store.writeAttempt).toHaveBeenCalledWith(
      'profile',
      { repoPathOrUrl: '/workspace', name: 'release' },
      expect.objectContaining({
        docHash: request.expectedDocHash,
        status: 'failed',
        pins: [{ url: 'https://example.test/box.git', commit: 'a'.repeat(40) }],
  ...
  it('writes a complete success snapshot with canonical step and hook hashes', async () => {
    const { service, jobs, store } = makeService();
    await service.start('profile', request);
    await expect(jobs.started[0].runner(ctx)).resolves.toMatchObject({
      sources: [{ id: 'box', status: 'ready' }],
    });
    expect(store.writeInstalled).toHaveBeenCalledWith(
      'profile',
      { repoPathOrUrl: '/workspace', name: 'release' },
      expect.objectContaining({
        docHash: request.expectedDocHash,
  ...
  it('keeps the hash read at start when the workflow changes while the job runs', async () => {
    ...
    expect(store.writeInstalled).toHaveBeenCalledWith(
      'profile',
      expect.any(Object),
      expect.objectContaining({ docHash: request.expectedDocHash }),
      expect.any(Function)
    );
  });
    ...
    (deps.jobs.list as ReturnType<typeof vi.fn>).mockReturnValue([
      {
        ...
        params: {
          ...
          docHash: request.expectedDocHash,
          pins: [
            { url: 'https://persisted.test/pin.git', commit: 'd'.repeat(40) },
          ],
        },
  ...
  it('treats a role mismatch as a terminal install failure', async () => {
    ...
    await service.start('profile', request);
    await expect(jobs.started[0].runner(ctx)).rejects.toMatchObject({
      details: { plugins: [{ id: 'foundry', status: 'wrong-type' }] },
    });
  });
});
