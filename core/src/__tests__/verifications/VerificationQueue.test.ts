...
describe('VerificationQueue', () => {
  ...
  it('queues a derived contract-type capture with its dedicated origin', async () => {
    ...
    const run = { id: 'run', plan: { contracts: [{ id: 'proxy', origin: 'contract-type', pluginId: 'ct', artifactKey: 'proxy' }] }, contractTypes: { ct: { unverifiedProvenance: undefined } }, inputs: { proxy: { bundleHash: 'unused' } }, explorerTargets: { '1': resolved.explorers } } as any;
  ...
  it('does not auto-submit an unverified-provenance capture', async () => {
    vi.useFakeTimers();
    const queue = await subject();
    (queue as any).bundles = { write: vi.fn(async () => 'b'.repeat(64)), read: vi.fn(async () => ({ })) };
    ...
    await queue.enqueueContractTypeCapture('p', { id: 'run', plan: { contracts: [{ id: 'proxy', origin: 'contract-type', pluginId: 'ct' }] }, contractTypes: { ct: { unverifiedProvenance: true } }, inputs: {}, explorerTargets: { '1': resolved.explorers } } as any, 1, 'wrapper', 'proxy', 'admin', '0x0000000000000000000000000000000000000002', artifact, '0x');
    expect(await queue.store.list('p')).toEqual([]);
  });
  it('fails closed when recovery has no frozen provenance entry', async () => {
    vi.useFakeTimers();
    const queue = await subject();
    const write = vi.fn(async () => 'b'.repeat(64));
    (queue as any).bundles = { write, read: vi.fn() };
    const logger = (queue as any).logger;
    const warn = vi.spyOn(logger, 'warn');
    await queue.enqueueContractTypeCapture('p', { id: 'run', plan: { contracts: [{ id: 'proxy', origin: 'contract-type', pluginId: 'ct' }] }, inputs: {}, explorerTargets: { '1': resolved.explorers } } as any, 1, 'wrapper', 'proxy', 'admin', '0x0000000000000000000000000000000000000002', { abi: [], creationBytecode: '0x6000', runtimeBytecode: '0x6001', solcVersion: '0.8.30', standardJsonInput: {}, sourceIdentifier: 'Admin.sol:Admin' } as any, '0x');
    expect(write).not.toHaveBeenCalled();
    expect(warn).toHaveBeenCalledWith(expect.stringContaining('frozen provenance is unavailable'));
  });
});
