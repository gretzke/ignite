...
function makePlan(overrides?: Partial<DeploymentPlan>): DeploymentPlan {
  return {
    schemaVersion: 1,
    contracts: [
      { id: 'c1', repoPathOrUrl: 'repo', frameworkId: 'f', artifactPath: 'a1', contractName: 'C1', sourcePath: 'C1.sol' },
      { id: 'c2', repoPathOrUrl: 'repo', frameworkId: 'f', artifactPath: 'a2', contractName: 'C2', sourcePath: 'C2.sol' },
    ],
    steps: [
...
function validated(plan: DeploymentPlan) {
  ...
  const checklist = { rpc: item, signers: item, args: item, estimation: item, balance: item, inputs: item };
  return {
    report: { chains: Object.fromEntries(plan.chains.map((chainId) => [String(chainId), checklist])) },
    frozen: Object.fromEntries(plan.contracts.map((contract) => [contract.id, {
      abi: [], creationBytecode: '0x6000' as const,
      compiler: { pluginId: 'f', version: '1', settingsHash: HASH },
      artifactHash: HASH, repoDirty: false,
    }])),
...
type Harness = {
  engine: DeployEngine;
  store: RunStore;
  executed: Array<{ chainId: number; data: string; overrides?: Record<string, unknown> }>;
...
describe('DeployEngine', () => {
  ...
  function makeEngine(deps?: Partial<DeployEngineDeps> & { capability?: 'sign-only' | 'sign-and-send' }): Harness {
    const store = (deps?.runStore as RunStore) ?? new RunStore({ baseDir: home });
    const harness: Harness = { engine: undefined as never, store, executed: [], artifactWrites: 0 };
    const engine = new DeployEngine({
      runStore: store,
      validate: async (plan) => validated(plan),
      resolveRpcUrl: async () => ({ url: 'http://rpc.local', fingerprint: HASH }),
      resolveAccount: async () => ({ account: { id: 'a', address: ADDRESS, capability: deps?.capability ?? 'sign-only' } }),
      chainMetadata: async () => ({ name: 'Anvil', nativeCurrency: { name: 'Ether', symbol: 'ETH', decimals: 18 } }),
      executeTx: async (args) => {
        harness.executed.push({ chainId: args.chainId, data: args.data, overrides: args.overrides as Record<string, unknown> });
        await args.onPhase?.('broadcasting', { tx: { nonce: harness.executed.length }, rawTx: RAW_TX, txHash: TX_HASH });
        return { txHash: TX_HASH, ...RECEIPT };
      },
      writeArtifact: async () => { harness.artifactWrites += 1; },
  ...
  async function seedPausedRun(harness: Harness, opts: { reason: PauseReason; submitted: boolean; chainId?: number }) {
    ...
    await harness.store.mutate('p1', run.id, (current) => {
      ...
      lane.steps[0].attempts = [{
        ...
        ...(opts.submitted ? { txHash: TX_HASH, rawTx: RAW_TX, nonce: 0 } : {}),
        error: 'seeded failure',
      }];
      lane.pause = { reason: opts.reason, stepIndex: 0, error: 'seeded failure', attemptId };
    });
    gate.release();
  ...
  it('runs lanes in parallel with strict in-lane sequencing and completes the run', async () => {
    ...
    const harness = makeEngine({
      executeTx: async (args) => {
        ...
        await args.onPhase?.('broadcasting', { tx: { nonce: index }, rawTx: RAW_TX, txHash: TX_HASH });
        return { txHash: TX_HASH, ...RECEIPT };
      },
    });
    ...
    for (const capability of ['sign-only', 'sign-and-send'] as const) {
      for (const reason of REASONS) {
        for (const submitted of [false, true]) {
          ...
          const { run, attemptId, chainId } = await seedPausedRun(harness, { reason, submitted });
          ...
          for (const action of ALL_ACTIONS.filter((entry) => !allowed.includes(entry))) {
            const cmd = { action, attemptId, commandId: crypto.randomUUID(), edits: {}, txHash: TX_HASH, gas: { maxFeePerGas: '2', maxPriorityFeePerGas: '1' } } as unknown as ResolveLaneRequest;
            await expect(
  ...
  it('replays an identical commandId without re-executing and 409s a different command on a consumed attempt', async () => {
    const harness = makeEngine();
    const { run, attemptId, chainId } = await seedPausedRun(harness, { reason: 'broadcast', submitted: false });
    const commandId = crypto.randomUUID();
    const first = await harness.engine.resolveLane('p1', run.id, chainId, { action: 'retry', attemptId, commandId });
    const executedAfterFirst = harness.executed.length;
    const replay = await harness.engine.resolveLane('p1', run.id, chainId, { action: 'retry', attemptId, commandId });
    expect(replay.id).toBe(first.id);
    expect(harness.executed.length).toBe(executedAfterFirst);
    await expect(
      harness.engine.resolveLane('p1', run.id, chainId, { action: 'skip', attemptId, commandId: crypto.randomUUID() })
  ...
  it('returns one run for two concurrent launches with the same idempotency key', async () => {
    ...
    const [a, b] = await Promise.all([
      harness.engine.launch({ profileId: 'p1', plan, rpcSelection: { '1': 'rpc' }, idempotencyKey: 'same' }),
      harness.engine.launch({ profileId: 'p1', plan, rpcSelection: { '1': 'rpc' }, idempotencyKey: 'same' }),
    ]);
  ...
  it('pauses with write-failure and never broadcasts when the intent write fails', async () => {
      ...
      get(target, prop, receiver) {
        ...
        return (profileId: string, runId: string, fn: (run: never) => void) =>
          target.mutate(profileId, runId, (run) => {
            fn(run as never);
            const lane = (run as { lanes: Record<string, { steps: Array<{ status: string }> }> }).lanes['1'];
    ...
    const harness = makeEngine({
      ...
      executeTx: async (args) => {
        await args.onPhase?.('broadcasting', { tx: { nonce: 0 }, rawTx: RAW_TX, txHash: TX_HASH });
        broadcasted = true;
        return { txHash: TX_HASH, ...RECEIPT };
      },
    });
  ...
  it('resume reconciles a persisted sign-only hash: mined confirms, unknown rebroadcasts the raw tx', async () => {
    ...
    const mined = await seedPausedRun(minedHarness, { reason: 'interrupted', submitted: true });
    ...
    const unknownHarness = makeEngine({
      ...
      rebroadcast: async () => { rebroadcasts += 1; receiptAvailable = true; return TX_HASH; },
    });
    const unknown = await seedPausedRun(unknownHarness, { reason: 'interrupted', submitted: true });
  ...
  it('confirm-hash verifies the receipt and continues; mark-not-sent enables a fresh retry', async () => {
    ...
    const seeded = await seedPausedRun(harness, { reason: 'needs-review', submitted: true });
    await harness.engine.resolveLane('p1', seeded.run.id, seeded.chainId, {
      action: 'confirm-hash', attemptId: seeded.attemptId, commandId: crypto.randomUUID(), txHash: TX_HASH,
    });
    const confirmed = (await harness.engine.get('p1', seeded.run.id))!;
    expect(confirmed.lanes['1'].steps[0]).toMatchObject({ status: 'confirmed', address: RECEIPT.contractAddress });

    const retryHarness = makeEngine({ capability: 'sign-and-send' });
    const second = await seedPausedRun(retryHarness, { reason: 'needs-review', submitted: true });
    await retryHarness.engine.resolveLane('p1', second.run.id, second.chainId, {
      action: 'mark-not-sent', attemptId: second.attemptId, commandId: crypto.randomUUID(),
    });
  ...
  it('replace pins the nonce and applies the edited gas to the next attempt', async () => {
    const harness = makeEngine();
    const seeded = await seedPausedRun(harness, { reason: 'receipt-timeout', submitted: true });
    await harness.engine.resolveLane('p1', seeded.run.id, seeded.chainId, {
      action: 'replace', attemptId: seeded.attemptId, commandId: crypto.randomUUID(),
  ...
  it('applies arg edits to the paused and later steps only, recording them on the attempt', async () => {
    ...
    await harness.engine.resolveLane('p1', seeded.run.id, seeded.chainId, {
      action: 'edit', attemptId: seeded.attemptId, commandId: crypto.randomUUID(),
      edits: { argsByStep: { 'step-1': { owner: ADDRESS }, 'step-2': { owner: ADDRESS } }, gas: { gasLimit: '400000' } },
    });
    const run = (await harness.engine.get('p1', seeded.run.id))!;
    expect(run.plan.steps[0].argsPerChain?.['1']).toMatchObject({ owner: ADDRESS });
    expect(run.plan.steps[1].argsPerChain?.['1']).toMatchObject({ owner: ADDRESS });
    expect(run.plan.steps[0].gasOverridesPerChain?.['1']).toMatchObject({ gasLimit: '400000' });
  ...
  it('records unresolvedTx when skipping a step whose tx may be in flight', async () => {
    ...
    const seeded = await seedPausedRun(harness, { reason: 'needs-review', submitted: true });
    await harness.engine.resolveLane('p1', seeded.run.id, seeded.chainId, {
      action: 'skip', attemptId: seeded.attemptId, commandId: crypto.randomUUID(), note: 'operator skipped',
    });
    ...
    expect(step.unresolvedTx).toMatchObject({ txHash: TX_HASH, note: 'operator skipped' });
  });
  ...
  it('sanitizes secret-bearing errors before they reach attempts, pauses, and events', async () => {
    ...
    const harness = makeEngine({
      executeTx: async () => { throw new Error('rpc https://rpc.example/SECRETKEY failed'); },
    });
  ...
  it('pauses with reason balance on an insufficient-funds failure', async () => {
    const harness = makeEngine({
      executeTx: async () => { throw new IgniteError('Account balance is too low for this transaction', ErrorCodes.INSUFFICIENT_FUNDS); },
    });
    const run = await launchDefault(harness, makePlan({ chains: [1] }));
    await eventually(async () => (await harness.engine.get('p1', run.id))?.lanes['1'].status === 'paused', 'paused');
  ...
  it('creates one immutable attempt per retry', async () => {
    ...
    const harness = makeEngine({
      executeTx: async (args) => {
        ...
        await args.onPhase?.('broadcasting', { tx: { nonce: 1 }, rawTx: RAW_TX, txHash: TX_HASH });
        return { txHash: TX_HASH, ...RECEIPT };
      },
    });
    const run = await launchDefault(harness, makePlan({ chains: [1] }));
    ...
    await harness.engine.resolveLane('p1', run.id, 1, { action: 'retry', attemptId: first.id, commandId: crypto.randomUUID() });
    await eventually(async () => (await harness.engine.get('p1', run.id))?.status === 'completed', 'completed');
    const attempts = (await harness.engine.get('p1', run.id))!.lanes['1'].steps[0].attempts;
    expect(attempts).toHaveLength(2);
    expect(attempts[0]).toMatchObject({ id: first.id, resolution: 'retry', error: 'first send failed' });
    expect(attempts[1].id).not.toBe(first.id);
  });

  it('aborting the run immediately terminates a paused lane and derives status failed', async () => {
    const harness = makeEngine({
      executeTx: async () => { throw new Error('send failed'); },
    });
    const run = await launchDefault(harness, makePlan({ chains: [1] }));
    await eventually(async () => (await harness.engine.get('p1', run.id))?.lanes['1'].status === 'paused', 'paused');
  ...
  it('rejects a launch whose authoritative validation has blocking failures', async () => {
    const harness = makeEngine({
      validate: async (plan) => {
        ...
        result.report.chains['1'] = { ...result.report.chains['1'], rpc: { ok: false, blocking: true, message: 'chain mismatch' } };
        return result;
      },
    });
