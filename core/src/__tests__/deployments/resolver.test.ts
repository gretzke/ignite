...
describe('deployment resolver', () => {
  it('uses positional keys for unnamed ABI inputs', () => {
    ...
    expect(missingArgKeys(inputs, { arg0: 'a', owner: address })).toEqual([
      'arg1',
    ]);
  });
  ...
  it('merges argument fields without deep-merging tuples or arrays', () => {
    const step = deployStep({
      args: { config: { owner: address, cap: '1' }, name: 'Token' },
      argsPerChain: { '1': { config: { owner: alternateAddress } } },
    });
    expect(mergeArgs(step, 1)).toEqual({
      config: { owner: alternateAddress },
      name: 'Token',
    });
  ...
  it('merges gas overrides field by field and resolves values as bigint', () => {
    const step = deployStep({
      value: '2',
      valuePerChain: { '1': '3' },
      gasOverrides: { gasLimit: '100', maxFeePerGas: '10' },
      gasOverridesPerChain: { '1': { maxPriorityFeePerGas: '1' } },
    });
    expect(mergeGas(step, 1)).toEqual({
      gasLimit: '100',
      maxFeePerGas: '10',
  ...
  it('resolves a per-chain signer without a global default and honors step exceptions', () => {
    ...
    const plan: DeploymentPlan = {
      ...
      steps: [],
      chains: [1, 2],
      signers: { perChain: { '1': chainSigner } },
    };
    const step = deployStep({
      signerOverride: { global: stepDefault, perChain: { '1': exception } },
    });
  ...
  it('orders and ABI-coerces constructor arguments recursively', () => {
    ...
    expect(
      toConstructorArgs(inputs, {
        supply: '42',
        owner: address,
        config: { recipient: alternateAddress, amounts: ['1', '2'] },
        pairs: [{ who: address, amount: '3' }],
      })
    ).toEqual([
      42n,
      address,
      { recipient: alternateAddress, amounts: [1n, 2n] },
      [{ who: address, amount: 3n }],
    ]);
    ...
    expect(() =>
      toConstructorArgs(inputs, {
        ...
        config: { recipient: alternateAddress, amounts: 'not-an-array' },
        pairs: [],
      })
    ).toThrow(/supply/);
  });
