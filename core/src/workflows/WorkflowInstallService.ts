...
export interface WorkflowInstallServiceDeps {
  readDocument: (repoPathOrUrl: string, name: string) => Promise<{
    document: WorkflowDocument;
    docHash: string;
  }>;
  jobs: Pick<JobManager, 'start' | 'get' | 'list'>;
  lifecycle: Pick<RepoLifecycle, 'runPinnedLifecycle'>;
  versionStore: Pick<VersionStore, 'isOriginApproved' | 'addMembership'>;
  registry: Pick<ProfileRepoRegistry, 'list'>;
  store: Pick<InstalledWorkflowStore, 'get' | 'writeInstalled' | 'writeAttempt'>;
  pluginStatus: (
    id: string,
...
export class WorkflowInstallService {
  ...
  async start(
    ...
    const loaded = await this.deps.readDocument(request.repoPathOrUrl, request.name);
  ...
  // Task 1.3 replaces this hook with the membership sweep.
  async sweep(profileId: string): Promise<void> {
    getLogger().debug(`Workflow membership sweep is not implemented yet for ${profileId}`);
  }

  async reconstructInterrupted(): Promise<void> {
    for (const job of this.deps.jobs.list()) {
      if (job.type !== 'workflow.install' || job.state !== 'failed' || job.error?.code !== 'INTERRUPTED') continue;
      const params = job.params as Record<string, unknown>;
      const profileId = typeof params.profileId === 'string' ? params.profileId : undefined;
      const repoPathOrUrl = typeof params.repoPathOrUrl === 'string' ? params.repoPathOrUrl : undefined;
      const name = typeof params.name === 'string' ? params.name : undefined;
      const docHash = typeof params.docHash === 'string' ? params.docHash : undefined;
      ...
      if (newer) continue;
      await this.deps.store.writeAttempt(profileId, { repoPathOrUrl, name }, {
        docHash,
        at: completedAt,
        status: 'interrupted',
  ...
  private async runAttempt(
    ...
    for (const source of document.sources) {
      if (source.origin === 'contract-type') {
        try {
          ...
          if (!(await this.deps.artifactReadable(source, profileId))) throw new Error('Contract-type artifact is not readable');
          sources.push({ id: source.id, status: 'ready' });
        } catch (error) {
          sources.push({ id: source.id, status: 'failed', reason: errorMessage(error), code: 'ARTIFACT_NOT_FOUND' });
        }
        continue;
      }
      try {
        ctx.log(`source ${source.id}: cloning\n`);
        await this.deps.versionStore.addMembership(profileId, source.repo.url, source.repo.commit, 'workflow');
        let lifecycle;
        try {
          lifecycle = await withLifecyclePermit(() => this.deps.lifecycle.runPinnedLifecycle(source.repo.url, source.repo.commit, profileId, ctx));
        } catch (error) {
          const coded = asCodedError(error);
          if (coded.code === 'PINNED_ORIGIN_UNAPPROVED') throw error;
          sources.push({ id: source.id, status: 'failed', reason: coded.message, code: 'LIFECYCLE_FAILED' });
          continue;
        }
        if (!lifecycle.frameworks.some((framework) => framework.id === source.frameworkId)) {
          sources.push({ id: source.id, status: 'failed', reason: `Framework '${source.frameworkId}' was not detected`, code: 'FRAMEWORK_MISSING' });
          continue;
        }
        try {
          ctx.log(`source ${source.id}: compiling\n`);
          if (!(await this.deps.artifactReadable(source, profileId))) throw new Error('Compiled artifact is not readable');
          sources.push({ id: source.id, status: 'ready' });
        } catch (error) {
          sources.push({ id: source.id, status: 'failed', reason: errorMessage(error), code: 'ARTIFACT_NOT_FOUND', artifactPath: source.artifactPath });
        }
      } catch (error) {
        const coded = asCodedError(error);
        if (coded.code === 'PINNED_ORIGIN_UNAPPROVED') {
          const origins = error && typeof error === 'object' && 'origins' in error && Array.isArray((error as { origins?: unknown }).origins)
            ? (error as { origins: string[] }).origins : [];
          throw Object.assign(new Error(coded.message), { code: 'PINNED_ORIGIN_UNAPPROVED', details: { origins } });
        }
        sources.push({ id: source.id, status: 'failed', reason: coded.message, code: 'LIFECYCLE_FAILED' });
      }
    }
    const plugins = await Promise.all(document.requiredPlugins.map((plugin) =>
      this.deps.pluginStatus(plugin.id, plugin.version, roles.get(plugin.id) ?? new Set()),
    ));
    const result: WorkflowInstallResult = { sources, plugins };
    if (sources.some((source) => source.status === 'failed') || plugins.some((plugin) => plugin.status !== 'installed')) {
      const failedSources = sources.flatMap((source) => source.status === 'failed' ? [{
        id: source.id,
        reason: source.reason,
        ...(source.code ? { code: source.code } : {}),
        ...(source.artifactPath ? { artifactPath: source.artifactPath } : {}),
      }] : []);
      await this.deps.store.writeAttempt(profileId, { repoPathOrUrl: request.repoPathOrUrl, name: request.name }, {
        docHash,
    ...
    const wrote = await this.deps.store.writeInstalled(
      ...
      { repoPathOrUrl: request.repoPathOrUrl, name: request.name },
      {
        ...
        plugins: document.requiredPlugins.map((plugin) => ({
          ...
          ...(plugin.source ? { source: this.canonicalJson(plugin.source) } : {}),
        })),
    ...
    await this.sweep(profileId);
    if (!wrote) {
      throw Object.assign(new Error('Repository was removed while workflow installation was running'), {
        code: 'REPO_NOT_FOUND',
        details: result,
      });
    }
  ...
  private async assertOriginsApproved(profileId: string, pins: Pin[]): Promise<void> {
    const origins = new Map<string, string>();
    for (const pin of pins) origins.set(pinnedOrigin(pin.url), pin.url);
    const unapproved = (await Promise.all([...origins].map(async ([origin, url]) => ({
      origin,
      approved: await this.deps.versionStore.isOriginApproved(profileId, url),
    })))).filter((entry) => !entry.approved).map((entry) => entry.origin);
    if (unapproved.length) {
      throw new WorkflowInstallServiceError(409, 'PINNED_ORIGIN_UNAPPROVED', 'Pinned origin approval required', { origins: unapproved });
    }
  }

  private async isRegistered(profileId: string, repoPathOrUrl: string): Promise<boolean> {
    const repos = await this.deps.registry.list(profileId);
    return [...repos.local, ...repos.cloned].some((repo) => repo.pathOrUrl === repoPathOrUrl);
  }

  private pinsFor(document: WorkflowDocument): Pin[] {
    return document.sources.flatMap((source) => source.origin === 'contract-type' ? [] : [{ url: source.repo.url, commit: source.repo.commit }]);
  }

  private validPins(value: unknown): Pin[] {
    if (!Array.isArray(value)) return [];
    return value.flatMap((pin) => pin && typeof pin === 'object' && typeof (pin as Pin).url === 'string' && typeof (pin as Pin).commit === 'string'
      ? [{ url: (pin as Pin).url, commit: (pin as Pin).commit }] : []);
  }

  private requiredRoles(document: WorkflowDocument): Map<string, Set<RequiredRole>> {
    ...
    for (const source of document.sources) {
      if (source.origin === 'contract-type') add(source.pluginId, PluginType.CONTRACT_TYPE);
      else add(source.frameworkId, PluginType.COMPILER);
    }
    for (const step of document.steps) {
      if (step.kind === 'deploy' && step.strategy?.kind === 'plugin') add(step.strategy.pluginId, PluginType.DEPLOYMENT_TYPE);
    }
    for (const hook of document.outputs.hooks) add(hook, PluginType.DEPLOYMENT_HOOK);
    return roles;
  }

  private snapshotSource(source: WorkflowSource): InstalledSourceSnapshot {
    if (source.origin === 'contract-type') {
      return { kind: 'contract-type', id: source.id, pluginId: source.pluginId, artifactKey: source.artifactKey, versionLabel: source.versionLabel, contentHash: source.contentHash };
    }
    return { kind: 'repo', id: source.id, pin: source.repo, frameworkId: source.frameworkId, sourcePath: source.sourcePath, contractName: source.contractName, artifactPath: source.artifactPath, ...(source.artifactHash ? { artifactHash: source.artifactHash } : {}) };
  }

  private failureMessage(result: WorkflowInstallResult): string {
    const failures = [
      ...result.sources.filter((source) => source.status === 'failed').map((source) => `${source.id}: ${source.reason}`),
      ...result.plugins.filter((plugin) => plugin.status !== 'installed').map((plugin) => `${plugin.id}: ${plugin.status}`),
    ];
