...
import {
  RepoService,
  type PromotionSourceInspection,
...
export interface WorkflowPromotionServiceDeps {
  ...
  withWorkflowWriteLock: <T>(
    repo: string,
    fn: (files: WorkflowFiles) => Promise<T>
  ) => Promise<T>;
...
export class WorkflowPromotionService {
  ...
  constructor(deps?: Partial<WorkflowPromotionServiceDeps>) {
    ...
    this.deps = {
      inspectSource:
        deps?.inspectSource ?? ((value) => repos.inspectPromotionSource(value)),
      readTargetFile:
        deps?.readTargetFile ??
        (async (repo, file) => {
      ...
      withWorkflowWriteLock:
        deps?.withWorkflowWriteLock ??
        ((repo, fn) => repos.withWorkflowWriteLock(repo, fn)),
      getRun:
        deps?.getRun ??
        ((profileId, runId) => new RunStore().get(profileId, runId)),
      ...
      renderRunArtifact:
        ...
        (async (profileId, runId) => {
          ...
          if (!run)
            throw new WorkflowPromotionError(
              404,
              'DEPLOYMENT_RUN_NOT_FOUND',
              `Deployment run not found: ${runId}`
            );
          const tasks = await VerificationQueue.getInstance().store.list(
            profileId,
            { runId }
          );
      ...
      freezeInputs:
        deps?.freezeInputs ??
        ((profileId, plan) =>
          new ArtifactFreezeService().freezeInputs(profileId, plan.contracts)),
      validateTargetRepo:
        deps?.validateTargetRepo ??
        ((repo) => repos.isExistingGitRepository(repo)),
      getVersionRecord:
        deps?.getVersionRecord ??
        ((url, commit) => new VersionStore().get(url, commit)),
    };
  }
  ...
  async promote(
    request: WorkflowPromoteRequest,
    profileId: string
  ): Promise<WorkflowPromoteData> {
    this.validateTarget(request.target);
    return request.mode === 'preview'
      ? this.preview(request, profileId)
      : this.apply(request, profileId);
  }
  ...
  private async preview(
    ...
    if (!(await this.deps.validateTargetRepo(request.target.repoPathOrUrl)))
      throw new WorkflowPromotionError(
        ...
        'Promotion target must be an existing git repository'
      );
    ...
    for (const source of plan.contracts) {
      ...
      if (source.pin) {
        sources.push({
          ...
          tagChoices:
            source.pin.refKind === 'tag' && source.pin.ref
              ? [source.pin.ref]
              : [],
          dirty: false,
        });
    ...
    const nameCollision =
      (await this.deps.readTargetFile(request.target.repoPathOrUrl, file)) !==
      null;
  ...
  private async apply(
    ...
    if (
      !snapshot ||
      snapshot.target !== targetKey(request.target) ||
      snapshot.inputKey !== inputKey(request)
    )
      throw new WorkflowPromotionError(
        409,
        'PROMOTION_PREVIEW_STALE',
        'Promotion preview is missing or no longer matches this request'
      );
    ...
    for (const source of plan.contracts) {
      ...
      if (!before)
        throw new WorkflowPromotionError(
          409,
          'PROMOTION_PREVIEW_STALE',
          `Source ${source.id} was not resolved by the preview`
        );
      ...
      try {
        ...
        throw new WorkflowPromotionError(
          409,
          'PROMOTION_PREVIEW_STALE',
          `Source ${source.id} can no longer be inspected`
        );
      }
      ...
      if (currentOrigin !== before.origin || current.commit !== before.commit)
        throw new WorkflowPromotionError(
          409,
          'PROMOTION_PREVIEW_STALE',
          `Source ${source.id} HEAD or origin changed since preview`
        );
      ...
      if (tags.length === 1) {
        ...
        if (!ref || !tags.includes(ref))
          throw new WorkflowPromotionError(
            422,
            'PROMOTION_TAG_CHOICE_REQUIRED',
            `Choose one tag for source ${source.id}`
          );
        refKind = 'tag';
    ...
    try {
      document = await this.buildDocument(
        plan,
        run,
        pins,
        request.hooks,
    ...
    await this.deps.withWorkflowWriteLock(
      request.target.repoPathOrUrl,
      async (files) => {
        const existing = await files.readFile(
          workflowPath(request.target.name)
        );
        if (existing !== null && !request.overwrite)
          throw new WorkflowPromotionError(
            409,
            'WORKFLOW_NAME_CONFLICT',
            `Workflow ${request.target.name} already exists`
          );
        await files.writeFile(workflowPath(request.target.name), raw);
        for (const runId of uniqueAdoptions) {
          await files.writeFile(
            `ignite/deployments/${request.target.name}/${runId}.json`,
            adoptedArtifacts.get(runId)!
          );
        }
      }
    );
  ...
  private async buildDocument(
    ...
    pins: Map<string, RepoWorkflowSource['repo']>,
    hooks: string[],
    profileId: string
  ): Promise<WorkflowDocument> {
    const frozen =
      run?.inputs ??
      (await this.deps.freezeInputs(profileId, plan).catch(() => undefined));
    ...
    const sources: WorkflowSource[] = plan.contracts.map((source) => {
      ...
      return {
        ...
        ...(frozen?.[source.id]?.artifactHash
          ? { artifactHash: frozen[source.id].artifactHash }
          : {}),
      };
    });
    const pluginIds = new Set<string>([
      ...sources.map((source) =>
        source.origin === 'contract-type' ? source.pluginId : source.frameworkId
      ),
      ...hooks,
    ]);
    for (const step of plan.steps)
      if (step.kind === 'deploy' && step.strategy?.kind === 'plugin')
        pluginIds.add(step.strategy.pluginId);
    const requiredPlugins = await Promise.all(
      [...pluginIds].sort().map((id) => this.deps.getRequiredPlugin(id))
    );
  ...
  private async localFallbackWarnings(
    plan: DeploymentPlan,
    pins: Map<string, RepoWorkflowSource['repo']>
  ): Promise<string[]> {
    ...
    for (const source of plan.contracts) {
      ...
      if (
        !pin ||
        !(await this.deps.getVersionRecord(pin.url, pin.commit))?.localFallback
      )
        continue;
      warnings.push(
        `The commit for ${source.contractName} is not on the remote and teammates cannot install it.`
      );
    }
    return warnings;
  }
  ...
  private async resolveInput(
    request: Pick<WorkflowPromoteRequest, 'plan' | 'runId'>,
    profileId: string
  ): Promise<{ plan: DeploymentPlan; run?: RunRecord }> {
    if (request.plan) return { plan: globalThis.structuredClone(request.plan) };
    if (!request.runId)
      throw new WorkflowPromotionError(
        400,
        'PROMOTION_INPUT_REQUIRED',
        'Exactly one of plan or runId is required'
      );
    ...
    if (!run)
      throw new WorkflowPromotionError(
        404,
        'DEPLOYMENT_RUN_NOT_FOUND',
        `Deployment run not found: ${request.runId}`
      );
  ...
  private validateTarget(target: {
    ...
    if (!WorkflowNamePattern.test(target.name))
      throw new WorkflowPromotionError(
        400,
...
async function requiredPlugin(id: string): Promise<WorkflowRequiredPlugin> {
  ...
  if (!config)
    throw new WorkflowPromotionError(
      422,
      'PROMOTION_PLUGIN_MISSING',
      `Required plugin is not installed: ${id}`
    );
  const source =
    config.origin === 'installed'
      ? await PluginManager.getInstance().getInstallSource(id)
      : undefined;
  return {
    id,
...
function inputKey(
  ...
  return request.runId
    ? `run:${request.runId}`
    : `plan:${crypto.createHash('sha256').update(JSON.stringify(request.plan)).digest('hex')}`;
}
...
function promotionOrigin(origin: string): string {
  ...
  try {
    ...
    if (
      url.protocol === 'https:' ||
      (process.env.NODE_ENV === 'development' && url.protocol === 'file:')
    )
      return normalized;
  } catch (error) {
    if (error instanceof WorkflowPromotionError) throw error;
  ...
  throw new WorkflowPromotionError(
    422,
    'PROMOTION_ORIGIN_UNSUPPORTED',
    'origin must be a credential-free HTTPS URL (or file:// in development)'
  );
}
...
function remapEncodeContractIds(
  value: unknown,
  sourceIds: Map<string, string>
): void {
  ...
  Object.values(record).forEach((entry) =>
    remapEncodeContractIds(entry, sourceIds)
  );
}
