...
export type DetectedExplorerEntry = { pluginId: string; chainId: number; url: string; label?: string };
...
export class VerifierProviderService {
  ...
  invalidate(pluginId?: string): void { pluginId ? this.cache.delete(pluginId) : this.cache.clear(); }
  ...
  private async fetch(id: string): Promise<Cache> { const controller = new AbortController(); const timer = setTimeout(() => controller.abort(), this.deps.timeoutMs); try { const response = await this.deps.execute(id, 'getSupportedExplorers', {}, { signal: controller.signal }); if (!response.success) { this.deps.logger.warn(`Verifier ${id} discovery failed (${response.error.code}): ${stripSentinelBlocks(response.error.message).slice(0, 200)}`); return {entries:[],patterns:[],state:'error',ts:this.deps.now()}; } const raw = response.data as { explorers?: unknown; urlPatterns?: unknown }; if (!raw || typeof raw !== 'object' || !Array.isArray(raw.urlPatterns)) return {entries:[],patterns:[],state:'error',ts:this.deps.now()}; const patterns = raw.urlPatterns.filter((x): x is string => typeof x === 'string').slice(0, 100); if (raw.explorers === null) return {entries:[],patterns,state:'needs-config',ts:this.deps.now()}; if (!Array.isArray(raw.explorers)) return {entries:[],patterns,state:'error',ts:this.deps.now()}; const entries: DetectedExplorerEntry[] = []; for (const item of raw.explorers.slice(0,500)) { if (!item || typeof item !== 'object') continue; const x = item as {chainId?:unknown;explorerUrl?:unknown;label?:unknown}; if (!Number.isInteger(x.chainId) || (x.chainId as number) <= 0 || typeof x.explorerUrl !== 'string') continue; try { entries.push({pluginId:id,chainId:x.chainId as number,url:normalizeExplorerUrl(x.explorerUrl),...(typeof x.label === 'string' && x.label.length <= 120 ? {label:x.label}: {})}); } catch { /* untrusted entry */ } } return {entries,patterns,state:'ok',ts:this.deps.now()}; } catch (error) { this.deps.logger.warn(`Verifier ${id} discovery threw: ${stripSentinelBlocks(error instanceof Error ? error.message : String(error)).slice(0,200)}`); return {entries:[],patterns:[],state:'error',ts:this.deps.now()}; } finally { clearTimeout(timer); } }
}
...
function defaultExecute(id:string, operation:string, options:Record<string,unknown>, opts?:{signal?:AbortSignal}) { return PluginExecutor.getInstance().execute(id, operation, options, opts); }
