...
export interface ClearedWorkflowRef {
  stepId: string;
  path: string;
}
...
function assertValid(doc: WorkflowDocument): WorkflowDocument {
  ...
  if (missing.length)
    throw new Error(
      `workflow closure is missing required plugins: ${missing.join(", ")}`,
    );
  return parsed;
}
...
function requiredStepRefs(step: WorkflowDocument["steps"][number]): string[] {
  ...
  return [step.target, ...Object.values(step.targetPerChain ?? {})].flatMap(
    (target) => (target.kind === "step" ? [target.stepId] : []),
  );
}

function clearArgReferences(
  value: unknown,
  path: string,
  removedStepIds: Set<string>,
  removedSourceId: string,
  clearedRefs: ClearedWorkflowRef[],
  clearStepRefs: boolean,
): void {
  ...
  if (Array.isArray(value)) {
    for (let index = value.length - 1; index >= 0; index -= 1) {
      ...
      if (clearStepRefs && isRemovedStepRef(entry, removedStepIds)) {
        clearedRefs.push({
          stepId: "",
          path: `${pathPart(path, index)}.$ref.stepId`,
        });
        value.splice(index, 1);
      } else if (
        isEncodeForSource(entry as Record<string, unknown>, removedSourceId)
      ) {
        clearedRefs.push({
          stepId: "",
          path: `${pathPart(path, index)}.$encode.contractId`,
        });
        value[index] = null;
      } else
        clearArgReferences(
...
function isRemovedStepRef(
  ...
  return (
    ...
    typeof ref === "object" &&
    !Array.isArray(ref) &&
    (ref as Record<string, unknown>).kind === "step" &&
    typeof (ref as Record<string, unknown>).stepId === "string" &&
    removedStepIds.has((ref as Record<string, unknown>).stepId as string)
  );
}
...
function isEncodeForSource(value: unknown, sourceId: string): boolean {
  ...
  return (
    ...
    (encode as Record<string, unknown>).contractId === sourceId
  );
}

function clearLibraries(
  step: Extract<WorkflowDocument["steps"][number], { kind: "deploy" }>,
  path: string,
  removedStepIds: Set<string>,
  clearedRefs: ClearedWorkflowRef[],
): void {
  for (const [name, binding] of Object.entries(step.libraries ?? {})) {
    if (binding.kind === "step" && removedStepIds.has(binding.stepId)) {
      clearedRefs.push({
        stepId: step.id,
        path: `${pathPart(pathPart(path, "libraries"), name)}.stepId`,
      });
      delete step.libraries![name];
    }
  }
  for (const [chain, libraries] of Object.entries(step.librariesPerChain ?? {}))
    for (const [name, binding] of Object.entries(libraries)) {
      if (binding.kind === "step" && removedStepIds.has(binding.stepId)) {
        clearedRefs.push({
          stepId: step.id,
          path: `${pathPart(pathPart(pathPart(path, "librariesPerChain"), chain), name)}.stepId`,
        });
...
export function cascadeRemoveSource(
  doc: WorkflowDocument,
  sourceId: string,
): {
  doc: WorkflowDocument;
  ...
  const removed = new Set(
    next.steps
      .filter((step) => step.kind === "deploy" && step.contractId === sourceId)
      .map((step) => step.id),
  );
  let changed = true;
  while (changed) {
    ...
    for (const step of next.steps)
      if (
        !removed.has(step.id) &&
        requiredStepRefs(step).some((id) => removed.has(id))
      ) {
        removed.add(step.id);
        changed = true;
      }
  ...
  for (let index = 0; index < next.steps.length; index += 1) {
    ...
    if (step.kind === "deploy")
      clearLibraries(step, stepPath, removed, clearedRefs);
    clearArgReferences(
      step.args,
      `${stepPath}.args`,
      removed,
    ...
    if (step.kind === "deploy" && step.strategy?.kind === "plugin")
      clearArgReferences(
        step.strategy.params,
        `${stepPath}.strategy.params`,
        removed,
        sourceId,
        clearedRefs,
        false,
  ...
  for (const cleared of clearedRefs)
    if (!cleared.stepId) {
...
): { doc: WorkflowDocument; sourceId: string; stepId: string } {
  const next = clone(doc);
  const sourceId = mintSourceId(next, source.contractName);
  const stepId = mintId(
    `deploy-${sourceId}`,
    new Set(next.steps.map((step) => step.id)),
  );
  next.sources.push({ ...clone(source), id: sourceId });
  next.steps.push({ id: stepId, kind: "deploy", contractId: sourceId });
  if (!next.requiredPlugins.some((plugin) => plugin.id === requiredPlugin.id))
    next.requiredPlugins.push(clone(requiredPlugin));
  return { doc: assertValid(next), sourceId, stepId };
}

export function changeSourceVersion(
  doc: WorkflowDocument,
  sourceId: string,
  pin: WorkflowPin,
): WorkflowDocument {
  ...
  if (!source || source.origin === "contract-type")
    throw new Error(`repo workflow source not found: ${sourceId}`);
